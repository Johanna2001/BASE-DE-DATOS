...
--SE PUEDAN VISUALIZAR LAS ACCIONES QUE SE REALIZENN EN LA BASE DE DATOS 
  ...
  CREATE OR REPLACE TRIGGER DETALLES 
   ...
   INSERT INTO DETALLE_ACCION (ID_DETALLE, ACCION, FECHA, USUARIO)
   VALUES('I-'||:NEW.ID_CLIENTE||:NEW.ID_NACIMIENTO ||:NEW.CIU_ID_NACIMIENTO ||:NEW.NOMBRE_CLIENTE,'SE HA INSERTADO ' ||:NEW.APELLIDOS_CLIENTE
   ||:NEW.FECHA_NACIMIENTO_CLIENTE ||:NEW.LUGAR_NACIMIENTO_CLIENTE ||:NEW.TELEFONO_CLIENTE ||:NEW.TELEFONO2_CLEINTE ||:NEW.TIPO_SANGRE_CLIENTE 
   ||:NEW.ESTADO_CLIENTE ||:NEW.CORREO_CLIENTE ||:NEW.POLIZA_ID, V_FECHA, V_USERNAME);
   END;
    
   INSERT INTO CLIENTE (id_cliente,id_nacimiento,ciu_id_nacimiento, nombre_cliente, apellidos_cliente,fecha_nacimiento_cliente,
   lugar_nacimiento_cliente,telefono_cliente,telefono2_cleinte,tipo_sangre_cliente,estado_cliente,correo_cliente,poliza_id)
   VALUES ( '16','16', '16', 'Jonas', 'Cabrera', (TO_DATE('12/01/1958', 'dd/mm/yyyy')), 'Manta', '0959897451',
   '093654785', 'A+', 'Activo', 'jonascabrera@gmail.com', '16');
   /*SELECT*FROM CLIENTE;*/
   
   /*TRIGGER 2*/   
--REALLIZAR UN TRIGGER CADA VEZ QUE SE ELIMINE UN NUEVO CLIENTE
--SE PUEDAN VISUALIZAR LAS ACCIONES QUE SE REALIZENN EN LA BASE DE DATOS
   CREATE OR REPLACE TRIGGER DETALLES
   ...
   SELECT SYSDATE INTO V_FECHA FROM DUAL;
   INSERT INTO DETALLE_ACCION (ID_DETALLE, ACCION, FECHA, USUARIO)
   VALUES('D-'||:OLD.ID_CLIENTE||:OLD.ID_NACIMIENTO ||:OLD.CIU_ID_NACIMIENTO ||:OLD.NOMBRE_CLIENTE,'SE HA ELIMINADO' ||:OLD.APELLIDOS_CLIENTE
   ||:OLD.LUGAR_NACIMIENTO_CLIENTE ||:OLD.TELEFONO_CLIENTE ||:OLD.TELEFONO2_CLEINTE ||:OLD.TIPO_SANGRE_CLIENTE ||:OLD.ESTADO_CLIENTE
   ||:OLD.CORREO_CLIENTE, V_FECHA, V_USERNAME);
   END;
   /*SELECT*FROM CLIENTE;
   DELETE FROM cliente WHERE nombre_cliente = 'Jhon';
   SELECT * FROM detalle_accion;*/
   
   /*CURSOR*/
   --BLOQUE QUE DECLARE UN CURSOR QUE TOME TODOS LOS REGISTROS QUE OCUPE LA TABLA
   
   SET SERVEROUTPUT ON;
   DECLARE CURSOR CURSOR_REGISTRO IS
   SELECT * FROM CLIENTE;
   FILA CURSOR_REGISTRO%ROWTYPE;
   BEGIN
   FOR FILA IN CURSOR_REGISTRO
   LOOP
   DBMS_OUTPUT.PUT_LINE(' El cliente '||FILA.APELLIDOS_CLIENTE  || ' ' || FILA.NOMBRE_CLIENTE || '  está ' || FILA.ESTADO_CLIENTE);
   END LOOP;
   END;
 ...
 || NOMBRE_CLIENTE|| ' ' || APELLIDOS_CLIENTE || ' Se encuentra: ' ||ESTADO_CLIENTE || ' El costo de la poliza es de: $' ||POLIZA_COSTO
into v_cadena
from agente_vendedor a JOIN VENTAS v
   ...
   WHERE p.ID_AGENTE_VENDEDOR = p_codemp;
   end;
